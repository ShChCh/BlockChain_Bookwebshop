...
contract ShopingItem {
    ...
    function buy_item(address seller_address,bytes32 item_id,int8 amount_of_buy,uint value,uint time) public returns(uint256){
        ...
        bytes32 buy_id = convert( buyer.trade_history_list.length+10);
        bytes32 sell_id = convert( seller.trade_history_list.length+10);
        ...
        buyer.trade_history[buy_id].trade_status =1;
        ...
        seller.trade_history[sell_id].trade_status =1;
        emit update_history();
        return 3;
    }
    function get_trade_history(uint256 i) public returns(uint256,bytes32,uint,int8,bytes32,bytes16,uint,int8,int8){
            var id = convert(i+10);
            ...
            return(i+10,accounts[accounts[msg.sender].trade_history[id].trader].nickname,
    ...
    function withdraw(uint256 value) public returns(bool){
        if(accounts[msg.sender].balance<value){
            return false;
        }
        accounts[msg.sender].balance-=value;
    ...
    // function change_trade_status(uint256 i,int8 status) public returns(bool){
    //     var id = convert(i);
    ...
    //     return true;
    // }
    function refund(uint256 i, int8 status) public returns(bool){
        var _id = convert(i);
        if(accounts[msg.sender].trade_history[_id].trade_status!=1){
             return false ;
        }
        accounts[msg.sender].trade_history[_id].trade_status=status;

        accounts[msg.sender].balance+=accounts[msg.sender].trade_history[_id].value;
        account temp=accounts[accounts[msg.sender].trade_history[_id].trader];
        temp.trade_history[_id].trade_status=status;
        temp.Selling_item[accounts[msg.sender].trade_history[_id].id].amount+=accounts[msg.sender].trade_history[_id].amount;
        emit update_balance();
        emit update_items();
        emit update_history();
        return true;
    }
    function finish_trade(uint256 i, int8 status) public returns(bool){
        var id = convert(i);
        if(accounts[msg.sender].trade_history[id].trade_status!=1){
            return false;
        }
        ...
        account temp=accounts[accounts[msg.sender].trade_history[id].trader];
        temp.trade_history[id].trade_status=status;
        emit update_balance();
        emit update_history();
        return true;
    }
